...
server.once("listening", async () => {
  ...
  try {
    ...
    const docPages = docs.files
      .filter(
        (doc) => doc.path.startsWith("docs/") && !doc.path.endsWith("/index.md")
      )
      .map((doc) => `/${doc.path.replace("docs/", "").replace(/\.md$/, "")}`);

    const toPrerender = ["/", "/changelog", ...docPages];
    ...
    for (const pathname of toPrerender) {
      ...
      for (const [script] of inlineScripts) {
        // const script = _script.innerHTML;
        if (script.startsWith(PREFIX) && script.endsWith(SUFFIX)) {
          rscPayload += JSON.parse(
            `"${unescapeScript(script.slice(PREFIX.length, -SUFFIX.length))}"`
          );
        }
      }
...
const SUFFIX = `")</script>`;
const OUTDIR = path.resolve("./dist/prerendered");
...
// }
function unescapeScript(script) {
  return script.replace(/<\\!--/g, "<!--").replace(/<\/\\(script)/gi, "</$1");
}
